...
// Informational only — describes what the backend WILL do, purely so the
// UI can show an accurate confirmation message before the call is made.
// The actual stock mutation happens server-side, atomically, inside the
// same transaction as the status update (see orderController.updateStatus).
// The frontend must never call productTypeStore.updateStock() directly for
// order transitions — that caused a double-reversal bug previously.
export function shouldReverseStock(from: OrderStatus, to: OrderStatus): boolean {
  if (from === to) return false;
  if (to === 'returned' && from === 'delivered') return true;
  return false;
