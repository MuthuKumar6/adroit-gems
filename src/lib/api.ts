...
const wrappedFetch = async (url: string, init: RequestInit = {}) => {
  ...
  try {
    ...
    // Network failure (offline, DNS, CORS, server unreachable) — fetch()
    // throws here rather than resolving, and previously nothing caught
    // it, so a dropped connection could crash whatever called this.
...
export const api = {
  ...
  bills: {
    ...
    void: (id: string, reason?: string) =>
      wrappedFetch(`${API_BASE}/bills/${id}/void`, { method: 'PATCH', headers: defaultHeaders(), body: JSON.stringify({ reason }) }),
  },
