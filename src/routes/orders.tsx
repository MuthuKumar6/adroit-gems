...
function OrdersPage() {
    ...
    try {
      ...
      setBilledOrderIds(new Set(
        (Array.isArray(billsData) ? billsData : [])
          ...
          .map((b: any) => b.order_id ?? b.orderId)
          .filter(Boolean)
      ));
    } catch (err) {
      console.error("Failed to fetch data:", err);
  ...
  // Stock deduction (on delivery) and restoration (on return) both happen
  // atomically on the backend inside the same transaction as the status
  // update — see orderController.updateStatus. The frontend no longer
  // makes a second, separate call to adjust stock (that was the source of
  // a prior double-reversal bug and a race condition between the two
  // requests).
  const applyStatusChange = async (current: Order, status: OrderStatus): Promise<boolean> => {
    const err = validateTransition(current, status, billedOrderIds);
    if (err) { alert(err); return false; }
    ...
    try {
      ...
      // Surface backend validation/lock errors (e.g. transition not
      // allowed, or order has an active bill) instead of failing silently.
      const msg = err?.response?.data?.error || err?.message || `Failed to change status to "${status}"`;
      alert(msg);
      return false;
    }
    return true;
              ...
              <div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {(detailOrder?.items ?? []).map((item: any) => {
                      const pt = productTypes.find(p => p.id === (item.product_type_id || item.productTypeId));
                      const weight = Number(item.weight_grams ?? item.weightGrams ?? 0);
                      const rate = Number(item.rate_per_gram ?? item.ratePerGram ?? 0);
                      const making = Number(item.making_charges ?? item.makingCharges ?? 0);
                      const amount = weight * rate + making;
                      return (
                        <TableRow key={item.id}>
                          <TableCell>{pt?.name || item.product_type_id || '—'}</TableCell>
