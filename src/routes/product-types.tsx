...
// import { Plus, Pencil, Trash2, Loader2, QrCode as QrCodeIcon, Printer, FileDown, ImageDown } from "lucide-react";
...
function ProductTypesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = editing
        ...
        : await productTypeStore.add(data);
      if (!res?.ok) {
        toast.error(res?.error || "Failed to save product type");
        return;
      }
      await fetchData();
      setDialogOpen(false);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this product type?")) return;
    try {
      const res = await productTypeStore.delete(id);
      if (!res?.ok) {
